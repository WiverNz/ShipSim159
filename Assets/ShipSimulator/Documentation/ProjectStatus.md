...
simulator built for several ship models, chosen in the start menu before a passage. Three vessels are
playable: a Project 507B Volgo-Don dry cargo ship, a Project 2-95A/R Volgo-Balt river-sea cargo ship
and a Project 1577 Volgoneft river-sea tanker.
...
| Vessels | 3 playable (Volgo-Don 507B, Volgo-Balt 2-95A/R, Volgoneft 1577), chosen in the start menu; KVLCC2 benchmark data for model checks |
| Passages | `GorodetsTrainingScene` (2.27 km mission, default) and `RiverTrainingScene` (familiarisation) |
| Automated tests | EditMode 141 passed, PlayMode 21 passed (latest recorded runs) |
...
| EditMode tests | 141 passed, 0 failed |
...
| `VirtualSeaTrials` | `SEA_TRIALS|PASS`: deep-water loaded results inside the IMO MSC.137(76) envelope for all three vessels |
...
1.19 times its draft, where depth effects on the hull are strongest.
...
- **Generated models are simple.** The Volgo-Balt and Volgoneft meshes are untextured and use
  estimated superstructure proportions and liveries; they are not built from drawings or photos.
