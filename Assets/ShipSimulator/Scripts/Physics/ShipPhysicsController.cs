...
namespace ShipSimulator.Physics
{
    ...
    public sealed class ShipPhysicsController : MonoBehaviour
    {
        ...
        public void Simulate(float dt)
        {
            ...
            diagnostics.SupportLiftN = SupportModel.Apply(parameters, body, transform,
                waterLevel, diagnostics.SupportFraction);
            ...
            if (SupportModel.Lifts(data.support))
                for (int i = 0; i < 4; i++)
                    effectiveDraft = Mathf.Max(effectiveDraft,
                        waterLevel - transform.TransformPoint(SupportModel.ContactPoint(parameters, i)).y);
        }
