...
- `Assets/ShipSimulator/Models/`: one folder per vessel. `VolgoDon507/` holds the imported FBX,
  ...
  materials that `Build Vessel Catalogue` overwrites. `Luch14352/` holds the authored Blender
  FBX and URP materials; its editable source is `Art/Source/Luch14352/Luch14352.blend`.
...
| `Integrate Luch 14352 Blender Model` | Re-imports the authored Luch model into its prefab |
| `Integrate Meteor 342U Blender Model` | Re-imports the authored Meteor model into its prefab |
...
The Luch 14352 and Meteor 342U visuals are authored in Blender. `Luch14352ModelIntegrator` and
`Meteor342UModelIntegrator` import their three FBX LODs each and update only the visual child and
model-dependent camera/light layout. Those branches of the catalogue builder delegate to the
integrators, retaining the existing physics root. See `Art/Source/Luch14352/README.md` and
`Art/Source/Meteor342U/README.md` for source references, axes and reconstruction limits.
